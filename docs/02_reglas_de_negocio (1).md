...
- **El evento de Calendar local (en el Event Log de OPERAL OS) es el canónico. Google Calendar es un espejo opcional.** Cuando se agenda o reagenda una llamada, el evento se guarda siempre en OPERAL OS — con o sin conexión a Google, y aunque la llamada a la API de Google falle en ese momento. La sincronización con Google es un efecto adicional, nunca un requisito para que el evento exista en OPERAL OS. Esto es lo que permite que la fecha de la llamada se pueda leer siempre desde nuestra base (por ejemplo, para calcular tiempos entre etapas), sin depender de que Google esté conectado en el momento de la consulta.
- **Un evento local sin sincronizar puede sincronizarse después** — al editarlo con conexión activa, o con la acción explícita "Sincronizar con Google". Ver `03_catalogo_eventos.md` evento 13 (`CALENDAR_EVENTO_SINCRONIZADO`).
- **Un lead tiene a lo sumo un evento de Calendar vigente.** Crear uno nuevo cuando ya existe uno vigente para ese lead se trata como reemplazo, no como un segundo evento paralelo — ver `03_catalogo_eventos.md` eventos 11, 12 y 13.
